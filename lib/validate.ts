...
export const validateDecision = (decision: Decision): string[] => {
  ...
  for (const factor of decision.factors) {
    ...
    if (
      ...
      factor.temporal_profile !== "steady_structural"
    ) {
      errors.push(`Factor '${factor.name || factor.id}' needs a valid temporal_profile`);
    }
    ...
    for (const pair of factor.axiom_pairs) {
      ...
      // Only enforce physical time shape for transition profiles
      if (factor.temporal_profile === "transition") {
        if (pair.time_type === "finite") {
          ...
          errors.push(`Invalid or missing time_type for transition factor ${factor.id}`);
        }
      } else if (factor.temporal_profile === "steady_case_flow") {
        // Case-flow requires duration_years for the per-case model
        if (pair.duration_years === undefined || pair.duration_years === null) {
          errors.push(
            `duration_years required for case_flow factor ${factor.id} (used in per-case calculation)`
          );
        }
      } else {
        // Steady structural: time_type not enforced
        // We silently ignore time_type for structural factors in scoring
      }
